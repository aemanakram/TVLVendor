            ...
            if(res["inventory"] != null) {
                ...
                if(partsInfo != null) {
                    for (part in partsInfo) {
                        ...
                        parts.add(Part(part["id"] as String?,null,null,null,null,
                            quantity = part["quantity"] as Number?, price = part["price"] as Number?))
                    }
                    ...
                    db.collection("Part").whereIn(idRef, partsIds).get().addOnSuccessListener { partSnapshot ->
                        for (doc in partSnapshot){
                            ...
                            if(index != -1)
                                parts[index] = Part(doc.id, doc["name"] as String?, doc["description"] as String?,
                                    doc["life"] as String?, doc["type"] as String?, parts[index].quantity,
                                    parts[index].price)
